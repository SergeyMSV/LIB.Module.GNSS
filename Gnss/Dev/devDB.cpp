...
void Create()
{
	...
	const std::vector<std::string> ReqList
	{
		...
		"CREATE TABLE sat(sat_id TINYINT UNSIGNED NOT NULL, plane TINYINT UNSIGNED, svn SMALLINT UNSIGNED, slot TINYINT SIGNED, prn TINYINT UNSIGNED, norad TINYINT UNSIGNED, cosmos SMALLINT UNSIGNED, type_ka VARCHAR(10), hsa BOOL NOT NULL DEFAULT FALSE, hse BOOL	NOT NULL DEFAULT FALSE, hse_datetime DATETIME, launch_date DATE, intro_date DATE, outage_date DATE, aesv SMALLINT UNSIGNED, update_date DATE, PRIMARY KEY(sat_id));"
	};
